...
{
	internal class Lz4DecodeStream : Stream
	{
		...
		internal Lz4DecodeStream(byte[] buffer, int offset, int length) : this(new MemoryStream(buffer, offset, length), length, false)
		{
		}
		...
		internal Lz4DecodeStream(Stream baseStream, bool leaveOpen = true) : this(baseStream, baseStream.Length, leaveOpen)
		{
		}
		...
		internal Lz4DecodeStream(Stream baseStream, long compressedSize, bool leaveOpen = true)
		{
			if (compressedSize <= 0)
			{
